...
{
	vec3 n = normal;
	float x = modeling_position.x + snoise(modeling_position * 2.0) / 4.0;
	//float x = position.x + snoise(position * 2.0) / 4.0;
	...
	return c * s_dot_n + (s_dot_n > 0 ? c * pow(max(dot(r, v), 0.0), shininess) : vec3(0.0));
}
