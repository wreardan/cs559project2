/*
Code taken from GLSL 4.0 Cookboook for Shadows, Chapter 7
Also based on the previous Spotlight shader, Spotlight_Wireframe Shader
...
void shadeWithShadow()
{
	...
	vec4 lit_color1 = vec4(ads(), 1.0) * t_color * vec4(Gcolor, 1.0);
	vec4 lit_color2 = vec4(adsWithSpotlight() * Gcolor, 1.0);
	vec4 lit_color = lit_color1 + lit_color2;

	float shadow = textureProj(shadow_map_texture, GShadowCoord);
	lit_color = lit_color * shadow;
